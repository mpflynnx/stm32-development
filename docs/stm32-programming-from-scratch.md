...
TARGET_FLAGS these are flags which are passed to the compiler. These can be found in the [documentation](https://gcc.gnu.org/onlinedocs/gcc-4.3.2//gcc/ARM-Options.html) for the compiler. This example if for a bare metal application (no cmsis)
...
Compilation flags are specified using CFLAGS.
...
Debugging is enabled `-g`, and all warnings are enabled `-Wall -Wextra - Werrors`. 
